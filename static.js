...
MongoClient.connect("mongodb://" + config.dbAddress, function(error, client) {
...
function serveApp(res) {
	res.cookie("server", config.serverAddress);
...
server.post("/register", function(req, res) {
	...
	accounts.registerQuick(data.name, function(account) {
		res.cookie("token", account.tokens[0].toString(), {
			maxAge: config.tokenMaxAge * millisecondsInDay
		});
		res.json({ok: true});
	});
});
...
server.listen(config.staticPort, function() {
	console.log("Server listening on port " + config.staticPort);
});
